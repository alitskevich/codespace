...
export class WordsStore extends StoredData {
  ...
  updateStemmAcquitance({ id, acquired }) {
    ...
    const newParsequence = (parsequence as any)?.map((st: any) => ({
      ...st,
      stemm: newData[st.stemm?.id],
    }));
  ...
  get textWords() {
    ...
    return markFilter === "all" ? list : list.filter(({ acquired }) => acquired == markFilter);
  ...
  get tree() {
    ...
    this.concepts?.forEach((e) => {
      ...
      const topicElt =
        domainElt.items[topic] ??
        (domainElt.items[topic] = { id: topic, name: topic, domain, items: {} });
      topicElt.items[id] = {
        ...info,
        id,
  ...
  get treeInterview() {
    ...
    this.interview?.forEach((e) => {
      ...
      const topicElt =
        domainElt.items[topic] ??
        (domainElt.items[topic] = { id: topic, name: topic, domain: kind, items: {} });
      topicElt.items[id] = {
        ...info,
        id,
        acquired: acquired?.[id]?.acquired ?? 0,
