...
export const stemm = function (s: string): Stemm {
  s = `${s}`;
  const l = s.length;
  if (l < 4) {
    return { id: s, names: { [s]: 1 } };
  }
...
export const cachedStemm = (s: string) => {
  return cache.has(s) ? cache.get(s) : cache.set(s, stemm(s)).get(s);
}