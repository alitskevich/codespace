...
import launchWeb, { defineCustomElement } from "arrmatura-web";
...
import { PreviewWebPlatform } from "./src/preview/PreviewWebPlatform";
...
function main() {
  ...
  fetchApiData(`projects`, projectsUrl).then(({ projects }) => {
    if (pageId === "project") {
      ...
      const { projectId, itemType, itemId, darkMode, locale } = params;
      const projectUrl = `https://script.google.com/macros/s/${projectId}/exec`;
