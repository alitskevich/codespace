import { Hash, StringHash, arrayGroupBy, arraySortBy, arrayToObject, urlParse } from "ultimus";
...
export const resolveProjectMetadata = async (key: string, url: string) => {
  const { props, pages, components, enums, consts, datasets, types, classes } = await fetchApiData(`project:${key}`, url);

  const propsHash = arrayGroupBy(
    props.map(({ prop, id, ...e }: Hash) => ({ ...e, id: prop })),
    "entity"
  );
  const data: any = {
...
${body}
  ...
  <Meta>
    ...
    <Properties>
      ${props?.map(({ type, id, ...e }: Hash) => `<Property id="${id}" type="${type}" />`).join("\n\t\t") ?? ""}
    </Properties>
  </Meta>
  </component>
    `)
...
`);
  ...
  const input = await loadFigmaFileSource('BAZJYlfhH81KeT4doDnjJM')//data.figmaFile); finance templates
