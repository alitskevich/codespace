...
(function (factory) {
...
})(($, Raphael, mousewheel, undefined) => {
  

  // The plugin name (used on several places)
  const pluginName = "mapael",
   ...
   */
   Mapael = function (container, options) {
    const self = this;
  ...
  Mapael.prototype = {
    ...
    init () {
      ...
      // Draw plots
      $.each(self.options.plots, (id) => {
        self.plots[id] = self.drawPlot(id);
      });

      // Attach zoom event
      self.$container.on(`zoom.${  pluginName}`, (e, zoomOptions) => {
      ...
      // Attach update event
      self.$container.on(`update.${  pluginName}`, (e, opt) => {
        self.onUpdateEvent(e, opt);
      });

      // Attach showElementsInRange event
      self.$container.on(`showElementsInRange.${  pluginName}`, (e, opt) => {
    ...
    destroy () {
      ...
      $.each(self.legends, (legendType) => {
        $.each(self.legends[legendType], (legendIndex) => {
          const legend = self.legends[legendType][legendIndex];
    ...
    initResponsiveSize () {
      ...
      // Function that actually handle the resizing
       handleResize = function (isInit) {
        const containerWidth = self.$map.width();

        if (self.paper.width !== containerWidth) {
          const newScale = containerWidth / self.mapConf.width;
          // Set new size
    ...
    extendDefaultOptions (options) {
      ...
      $.each(["area", "plot"], (key, type) => {
        if ($.isArray(options.legend[type])) {
          for (let i = 0; i < options.legend[type].length; ++i)
      ...
      self.$container.on(`mouseover.${  pluginName}`, "[data-id]", function () {
        ...
        mapMouseOverTimeoutID = setTimeout(() => {
          ...
          if (dataTypeToElementMapping[type] !== undefined) {
            ...
            const legendIndex = $elem.attr("data-legend-id"),
             legendType = $elem.attr("data-legend-type");
      ...
      $.each(self.customEventHandlers, (eventName) => {
        ...
        self.$container.off(fullEventName).on(fullEventName, "[data-id]", function (e) {
          ...
          if (
            ...
            const elem = self.customEventHandlers[eventName][type][id];
     ...
     */
    initElem (id, type, elem) {
      const self = this,
       $mapElem = $(elem.mapElem.node);
      ...
      if (elem.options.text && elem.options.text.content !== undefined) {
        ...
        const textPosition = self.getTextPosition(
     ...
     * @param zoomOptions
     */
    initZoom (mapWidth, mapHeight, zoomOptions) {
      let self = this,
       mousedown = false,
       previousX = 0,
       previousY = 0,
      ...
      // Init zoom buttons
      $.each(zoomOptions.buttons, (type, opt) => {
        if (fnZoomButtons[type] === undefined) {throw new Error(`Unknown zoom button '${  type  }'`);}
        // Create div with classes, contents and title (for tooltip)
        const $button = $("<div>").addClass(opt.cssClass).html(opt.content).attr("title", opt.title);
        // Assign click event
      ...
      if (self.options.map.zoom.mousewheel) {
        self.$map.on(`mousewheel.${  pluginName}`, (e) => {
          const zoomLevel = e.deltaY > 0 ? 1 : -1,
           coord = self.mapPagePositionToXY(e.pageX, e.pageY);
      ...
      if (self.options.map.zoom.touch) {
        self.$map.on(`touchstart.${  pluginName}`, (e) => {
          if (e.originalEvent.touches.length === 2) {
            self.zoomCenterX = (e.originalEvent.touches[0].pageX + e.originalEvent.touches[1].pageX) / 2;
            self.zoomCenterY = (e.originalEvent.touches[0].pageY + e.originalEvent.touches[1].pageY) / 2;
            self.previousPinchDist = Math.sqrt(
              (e.originalEvent.touches[1].pageX - e.originalEvent.touches[0].pageX)**2 +
                (e.originalEvent.touches[1].pageY - e.originalEvent.touches[0].pageY)**2
            );
          }
        });

        self.$map.on(`touchmove.${  pluginName}`, (e) => {
          let pinchDist = 0,
           zoomLevel = 0;
          ...
          if (e.originalEvent.touches.length === 2) {
            ...
            if (Math.abs(pinchDist - self.previousPinchDist) > 15) {
              const coord = self.mapPagePositionToXY(self.zoomCenterX, self.zoomCenterY);
      ...
      self.$map.on("dragstart", () => false);

      // Panning
      let panningMouseMoveTO = null,
       panningMouseUpTO = null;
      $("body").on(`mouseup.${  pluginName  }${zoomOptions.touch ? ` touchend.${  pluginName}` : ""}`, () => {
        mousedown = false;
      ...
      self.$map
        .on(`mousedown.${  pluginName  }${zoomOptions.touch ? ` touchstart.${  pluginName}` : ""}`, (e) => {
          clearTimeout(panningMouseUpTO);
          clearTimeout(panningMouseMoveTO);
        ...
        .on(`mousemove.${  pluginName  }${zoomOptions.touch ? ` touchmove.${  pluginName}` : ""}`, (e) => {
          let currentLevel = self.zoomData.zoomLevel,
           pageX = 0,
           pageY = 0;
          ...
          if (mousedown && currentLevel !== 0) {
            const offsetX = ((previousX - pageX) / (1 + currentLevel * zoomOptions.step)) * (mapWidth / self.paper.width),
             offsetY =
              ((previousY - pageY) / (1 + currentLevel * zoomOptions.step)) * (mapHeight / self.paper.height),
             panX = Math.min(Math.max(0, self.currentViewBox.x + offsetX), mapWidth - self.currentViewBox.w),
             panY = Math.min(Math.max(0, self.currentViewBox.y + offsetY), mapHeight - self.currentViewBox.h);
     ...
     */
    mapPagePositionToXY (pageX, pageY) {
      const self = this,
       offset = self.$map.offset(),
       initFactor = self.options.map.width
    ...
    onZoomEvent (e, zoomOptions) {
      ...
      // Zoom level in absolute scale (from 0 to max, by step of 1)
       {zoomLevel} = self.zoomData,

      // Relative zoom level (from 1 to max, by step of 0.25 (default))
       previousRelativeZoomLevel = 1 + self.zoomData.zoomLevel * self.options.map.zoom.step,
       relativeZoomLevel,

       animDuration =
        zoomOptions.animDuration !== undefined ? zoomOptions.animDuration : self.options.map.zoom.animDuration;
      ...
      if (zoomOptions.area !== undefined) {
        ...
        const areaMargin = zoomOptions.areaMargin !== undefined ? zoomOptions.areaMargin : 10,
         areaBBox = self.areas[zoomOptions.area].mapElem.getBBox(),
         areaFullWidth = areaBBox.width + 2 * areaMargin,
         areaFullHeight = areaBBox.height + 2 * areaMargin;
        ...
        if (zoomOptions.plot !== undefined) {
          ...
          if (zoomOptions.latitude !== undefined && zoomOptions.longitude !== undefined) {
            const coords = self.mapConf.getCoords(zoomOptions.latitude, zoomOptions.longitude);
      ...
      if (zoomLevel === 0) {
        ...
        if (zoomOptions.fixedCenter !== undefined && zoomOptions.fixedCenter === true) {
          panX =
            self.zoomData.panX +
            ((zoomOptions.x - self.zoomData.panX) * (relativeZoomLevel - previousRelativeZoomLevel)) /
              relativeZoomLevel;
          panY =
            self.zoomData.panY +
            ((zoomOptions.y - self.zoomData.panY) * (relativeZoomLevel - previousRelativeZoomLevel)) /
              relativeZoomLevel;
        } else {
     ...
     */
    showElemByRange (ranges, elems, hiddenOpacity, animDuration) {
      const self = this,
      /*
      ...
      $.each(ranges, (valueIndex) => {
        ...
        $.each(elems, (id) => {
          let elemValue = elems[id].options.value;
     ...
     */
    setElementOpacity (elem, opacity, animDuration) {
      const self = this;
      ...
      self.animate(elem.mapElem, { opacity }, animDuration, () => {
        // If final attribute is 0, hide
        if (opacity === 0) {elem.mapElem.hide();}
      });

      self.animate(elem.textElem, { opacity }, animDuration, () => {
        // If final attribute is 0, hide
    ...
    onUpdateEvent (e, opt) {
       ...
       */
       fnRemoveElement = function (elem) {
        self.animate(elem.mapElem, { opacity: 0 }, animDuration, () => {
          elem.mapElem.remove();
        });

        self.animate(elem.textElem, { opacity: 0 }, animDuration, () => {
      ...
      if (typeof opt.mapOptions === "object") {
        ...
        if (
          ...
          $("[data-type='legend-elem']", self.$container).each((id, elem) => {
      ...
      if (typeof opt.newLinks === "object") {
        const newLinks = self.drawLinksCollection(opt.newLinks);
      ...
      if (typeof opt.setLegendElemsState === "object") {
        ...
        $.each(opt.setLegendElemsState, (legendCSSClass, action) => {
          // Search for the legend
          const $legend = self.$container.find(`.${  legendCSSClass}`)[0];
          if ($legend !== undefined) {
            // Select all elem inside this legend
            $("[data-type='legend-elem']", $legend).each((id, elem) => {
              if (
         ...
         * Default : "show"
         */
        const action = opt.setLegendElemsState === "hide" ? "hide" : "show";
    ...
    setPlotCoords (plot) {
      ...
      if (plot.options.x !== undefined && plot.options.y !== undefined) {
        ...
        const areaBBox = self.areas[plot.options.plotsOn].mapElem.getBBox();
        plot.coords = {
          x: areaBBox.cx,
          y: areaBBox.cy,
        };
     ...
     */
    drawLinksCollection (linksCollection) {
      let self = this,
       p1 = {},
       p2 = {},
       coordsP1 = {},
      ...
      $.each(linksCollection, (id) => {
        const elemOptions = self.getElemOptions(self.options.map.defaultLink, linksCollection[id], {});
        ...
        if (p1.plotsOn !== undefined && self.areas[p1.plotsOn] !== undefined) {
          const p1BBox = self.areas[p1.plotsOn].mapElem.getBBox();
          coordsP1 = {
            x: p1BBox.cx,
            y: p1BBox.cy,
          };
        ...
        if (p2.plotsOn !== undefined && self.areas[p2.plotsOn] !== undefined) {
          const p2BBox = self.areas[p2.plotsOn].mapElem.getBBox();
          coordsP2 = {
            x: p2BBox.cx,
            y: p2BBox.cy,
          };
     ...
     */
    drawLink (id, xa, ya, xb, yb, elemOptions) {
      let self = this,
       link = {
        options: elemOptions,
      },
     ...
     */
    isAttrsChanged (originalAttrs, newAttrs) {
    ...
    updateElem (elem, animDuration) {
      ...
      if (elem.textElem) {
        ...
        const textPosition = self.getTextPosition(mapElemBBox, elem.options.text.position, elem.options.text.margin);
        elem.options.text.attrs.x = textPosition.x;
        elem.options.text.attrs.y = textPosition.y;
        elem.options.text.attrs["text-anchor"] = textPosition.textAnchor;
     ...
     */
    setEventHandlers (id, type, elem) {
      const self = this;
      $.each(elem.options.eventHandlers, (event) => {
     ...
     */
    drawLegend (legendOptions, legendType, elems, scale, legendIndex) {
      let self = this,
       $legend = {},
       legendPaper = {},
       width = 0,
      ...
      const initialHTMLContent = $legend.html();
      $legend.empty();
      ...
      for (i = 0; i < legendOptions.slices.length; ++i) {
        ...
        } else if (sliceOptions[i].attrs.r === undefined) {sliceOptions[i].attrs.r = sliceOptions[i].size / 2;}
      ...
      for (i = 0; i < sliceOptions.length; ++i) {
        ...
        if (sliceOptions[i].display === undefined || sliceOptions[i].display === true) {
          ...
          if (legendOptions.mode === "horizontal") {
            let currentHeight = legendOptions.marginBottom + legendElemBBox.height;
            width +=
     ...
     */
    handleClickOnLegendElem (elem, id, legendIndex, legendType, opts) {
      let self = this,
       legendOptions;
      opts ||= {};
      ...
      const legendElem = elem.mapElem,
       legendLabel = elem.textElem,
       $legendElem = $(legendElem.node),
       $legendLabel = $(legendLabel.node),
       sliceOptions = legendOptions.slices[id],
       mapElems = legendType === "area" ? self.areas : self.plots,
      // Check animDuration: if not set, this is a regular click, use the value specified in options
       animDuration =
        opts.animDuration !== undefined ? opts.animDuration : legendOptions.hideElemsOnClick.animDuration,

       hidden = $legendElem.attr("data-hidden"),
      ...
      $.each(mapElems, (y) => {
         ...
         */
         hiddenBy = mapElems[y].mapElem.data("hidden-by");
     ...
     */
    createLegends (legendType, elems, scale) {
      let self = this,
      ...
      for (let j = 0; j < legendsOptions.length; ++j) {
        if (
     ...
     */
    setHoverOptions (elem, originalAttrs, attrsHover) {
     ...
     * @param legendOptions
     */
    getElemOptions (defaultOptions, elemOptions, legendOptions) {
      let self = this,
       options = $.extend(true, {}, defaultOptions, elemOptions);
     ...
     */
    getTextPosition (bbox, textPosition, margin) {
      let textAnchor = "",
       textX = 0,
       textY = 0;
     ...
     */
    getLegendSlice (value, legend) {
      for (let i = 0; i < legend.slices.length; ++i) {
        if (
     ...
     */
    animateViewBox (targetX, targetY, targetW, targetH, duration, easingFunction) {
      let self = this,

       cx = self.currentViewBox.x,
       dx = targetX - cx,
       cy = self.currentViewBox.y,
       dy = targetY - cy,
       cw = self.currentViewBox.w,
       dw = targetW - cw,
       ch = self.currentViewBox.h,
       dh = targetH - ch;
      ...
      const zoomDir = cw > targetW ? "in" : "out",

       easingFormula = Raphael.easing_formulas[easingFunction || "linear"],

      // To avoid another frame when elapsed time approach end (2%)
       durationWithMargin = duration - (duration * 2) / 100,

      // Save current zoomAnimStartTime before assigning a new one
       oldZoomAnimStartTime = self.zoomAnimStartTime;
       ...
       */
      const computeNextStep = function () {
        /*
        ...
        const elapsed = new Date().getTime() - self.zoomAnimStartTime;
        ...
        if (elapsed < durationWithMargin) {
          ...
          if (oldZoomAnimStartTime && self.zoomAnimCVBTarget && self.zoomAnimCVBTarget.w !== targetW) {
            ...
            const realElapsed = new Date().getTime() - oldZoomAnimStartTime,
            // Compute then the actual ratio we're at
             realRatio = easingFormula(realElapsed / duration);
            /*
             ...
             */
            const ratio = easingFormula(elapsed / duration);
     ...
     */
    animate (element, attrs, duration, callback) {
      const self = this;
      // Check element
      if (!element) {return;}
      if (duration > 0) {
         ...
         */
        const attrsNonAnimated = {};
        for (let i = 0; i < self._nonAnimatedAttrs.length; i++) {
          const attrName = self._nonAnimatedAttrs[i];
        ...
        element.animate(attrs, duration, "linear", () => {
     ...
     */
    isRaphaelBBoxBugPresent () {
      const self = this,
      // Draw text, then get its boundaries
       textElem = self.paper.text(-50, -50, "TEST"),
       textElemBBox = textElem.getBBox();
      // Remove element
      textElem.remove();
