...
const compileArgument = (a: string, strings?: string[]) => {
  if (a[0] === "!" && a[1] === "@") {
    const getter = compilePropGet(a.slice(2));
  ...
  if (a[0] !== "@") return compileConstant(a);

  return compilePropGet(a.slice(1));
};
...
function withHardValue(hardValue: ExpressionText, pipec: PipeFn): PipeFn {
  ...
  const iniFn = hardValue[0] === "@" || hardValue[0] === "!" ? compilePlaceholder(hardValue) : compileConstant(hardValue);
...
export function compilePlaceholder(expr: string): Fn {
  ...
  if (key[0] === "'") {
    fn = compileConstant(key);
  } else if (key[0] === "@" || key[0] === "!") {
    fn = compileArgument(key);
  } else {
    console.warn(`Invalid placeholder: missed @-prefix: "${expr}"`);
    fn = compileArgument(`@${key}`);
  }
